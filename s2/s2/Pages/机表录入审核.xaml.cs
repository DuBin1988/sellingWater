...
{
    public partial class 机表录入审核 : UserControl
    {
            ...
            else
            {
                ...
                foreach (JsonObject json in items)
                {
                    ...
                    try
                    {
                        ...
                        decimal lastrecord = (decimal)json["lastrecord"];//本期指数
                        go.SetPropertyValue("lastrecord", lastrecord, false);
                        decimal oughtamount = (decimal)json["oughtamount"];//用水量
                        go.SetPropertyValue("oughtamount", oughtamount, false);
                        list.Add(go);
                    }
                    catch (Exception ex)
