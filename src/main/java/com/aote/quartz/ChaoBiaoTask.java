		...
		*/
		...
		try
		{
			...
			iesGas.chaobiaodan("insert into t_handplan(f_userid, f_username, lastinputgasnum, f_gaswatchbrand, f_metertype, "+
								...
								"f_zerenbumen, f_state, shifoujiaofei, users, f_cusDom, f_cusDy,f_gasmeterstyle,f_aliasname,zhye,f_userinfoid,f_handid,f_extrawaterprice,f_stairtype,f_meternumber) "+
								...
								"f_zerenbumen, '未抄表', '否', id, f_cusDom, f_cusDy,f_gasmeterstyle,f_aliasname,IsNull(f_zhye,0)f_zhye,f_userinfoid,f_handid,f_extrawaterprice,f_stairtype,f_meternumber "+
								"from t_userfiles where f_userid is not null and f_userstate='正常' and "+iesGas.gasmeterstyle+" and f_userid not in "+
								"(select distinct f_userid from t_handplan where f_state='未抄表') and f_userid not in "+
								"(select distinct f_userid from t_handplan where f_state='已抄表' and SUBSTRING(CONVERT(varchar(100),f_inputdate,25),1,13)='"+cbdate.format(now)+" 06') "+" ");	
		...
		try
		{
			...
			if(!"[]".equals(json))
				HandCharge.afRecordInputForMore(json,"水务公司","抄表系统",fmt.format(now),handdate.format(now),"正常使用");
