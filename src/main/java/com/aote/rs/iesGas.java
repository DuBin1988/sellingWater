...
public class iesGas {
	...
	//单价调整 ---------------------------------------------------------------------------------------------------->
		...
		//同步小区
		@Path("areacomand/{f_typesun}")
		@POST
		@Produces("application/json")
		public String areacomand(String Obj
				, @PathParam("f_typesun") String f_typesun // 类型：建档，充值，阀门
				) throws Exception {
			log.debug("同步用户档案：" + Obj);
			return jsonpost(7,Obj,"createArea","");
		}
				...
				switch (type)
				{
				   ...
				   case 6: //如果同步的是补费、扣费，，读取下数据库状态是1的，添加到请求数据中
					   try{
					   	...
					   	jsonString=rows.toString();
				       	break;
				   case 7: //如果同步的是表指令，，读取下数据库状态是1的，添加到请求数据中
					   try{
						   List list=null;
						   if("[]".equals(Obj)){
							   list=relList("select f_road,f_districtname from t_area where f_districtname is not null");								
						   }else{
							   JSONObject rowid = rows.getJSONObject(0);
							   list=relList("select f_road,f_districtname from t_area where f_districtname is not null and f_districtname='"+rowid.getString("f_districtname")+"'");
						   }
						   rows=new JSONArray();
						   // 查找数据记录是否存在
						   if (null != list && list.size() >= 1){
								for(int i=0;i<list.size();i++){
									Object[] tmp1 = (Object[])list.get(i);
									String parent_area= tmp1[0]+"";
									String child_area= tmp1[1]+"";									
									rows.put(new JSONObject("{type:\"0\",child_area:\"" + child_area + "\",parent_area:\"" + parent_area + "\"}"));
								}
						    }
						}catch(Exception e){}
					   jsonString=rows.toString();
