...
public class SellSer {
	...
	public String txSell(@PathParam("userid") String userid,
		...
		try {
			...
			for (Map<String, Object> map : list) {
				// 取出应交金额
				String h = (map.get("f_fee") + "");
				if (h.equals("null")) {
					h = "0.0";
				} else {
				}
				BigDecimal fee = new BigDecimal(h);
				fee = fee.setScale(2, BigDecimal.ROUND_HALF_UP);
				// 当前用户实际缴费够交，则扣除，交费记录变为已交
				int equals = total.compareTo(fee);// 判断total和fee的大小
				if (equals >= 0) {
					// 扣费，并产生本次余额
			...
			sell.put("f_pregas", gasSum.setScale(1, BigDecimal.ROUND_HALF_UP)
					.doubleValue()); // 气量
			BigDecimal oughtfee = new BigDecimal(userinfo.get("oughtfee") + "");
			sell.put("f_preamount", oughtfee.setScale(2, BigDecimal.ROUND_HALF_UP)
			 		.doubleValue()); // 气费
			BigDecimal extrazjfee = new BigDecimal(userinfo.get("extrazjfee") + "");
			sell.put("f_fjamount", extrazjfee.setScale(2, BigDecimal.ROUND_HALF_UP)
					.doubleValue()); // 附加费用
	...
	private void financedetailSave(int handId, String f_userid,
		...
		// <!--应收金额-->
		finance.put("f_fee", ysMoney.doubleValue());
		// 单价
	...
	private List<Map<String, Object>> findHanplans(String userid) {
		String sql = " select u.f_zhye f_zhye, u.f_accountzhye f_accountzhye, u.f_username f_username,u.f_cardid f_cardid, u.f_address f_address,u.f_districtname f_districtname,u.f_cusDom f_cusDom,u.f_cusDy f_cusDy,u.f_beginfee f_beginfee, u.f_metergasnums f_metergasnums, u.f_cumulativepurchase f_cumulativepurchase,"
				...
				+ "u.f_gasproperties f_gasproperties, u.f_userid f_userid, h.id handid, h.oughtamount oughtamount,h.extrazjfee extrazjfee,h.oughtfee oughtfee, h.lastinputgasnum lastinputgasnum,"
				+ "h.lastrecord lastrecord, h.shifoujiaofei shifoujiaofei, h.f_fee f_fee,h.f_debtmoney  f_debtmoney ,h.lastinputdate from t_userfiles u "
