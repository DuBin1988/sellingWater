...
public class SellSer {
	...
	public String getUserBill(@PathParam("userid") String userid)
		...
		String sql = "select "
				...
				+ ", isnull(h.f_stair4amount,0) f_stair4amount, isnull(h.f_stair4price,0) f_stair4price, isnull(h.f_stair4fee,0) f_stair4fee"
				+ // h t_handplan
		...
		result += ",f_districtname:'" + (String) userinfo.get("f_districtname")
				+ "'";
		result += ",f_inputtor:'" + (String) userinfo.get("f_inputtor")
				+ "'";
	...
	public String insertSell(Map<String, Object> userMap,
		...
		sale.put("f_inputtor", userMap.get("f_inputtor"));
		...
		sale.put("f_stair4price", inforMap.get("f_stair3price"));
		sale.put("f_stair4amount", inforMap.get("f_stair3amount"));
		sale.put("f_stair4fee", inforMap.get("f_stair3fee"));
		...
		String result = "{id:" + sellId + ", f_deliverydate:'" + f2.format(now)
				...
				+ "f_stair4price:" + inforMap.get("f_stair4price")
				+ ", f_stair4amount:" + inforMap.get("f_stair4amount")
				+ ", f_stair4fee:" + inforMap.get("f_stair4fee") + ", ";
		// 更新抄表记录sellid
	...
	private void updateUser(Map user, BigDecimal nowye, BigDecimal debtGas,
		...
		String usertypeString = user.get("f_usertype") + "";

		int zherownum;

		if (usertypeString.equals("民用")) {
			if (f_zherownum == "" || user.get("f_zherownum") == null || f_zherownum.equals("9")) {
				f_zherownum = "10";
			}
			zherownum = Integer.parseInt(f_zherownum);
			// 折子行号为22，换行
			if (zherownum >= 22) {
				zherownum = -2;
			}
			...
			String sql = "update t_userinfo  set f_zhye=" + nowye
					...
					+ (zherownum + 3) + " where f_userid='" + user.get("f_userid")
					+ "'";
	...
	private Map<String, Object> getInfor(String userinfoId) {
		final String sql = "select f_stair1price f_stair1price, f_stair1amount f_stair1amount, f_stair1fee f_stair1fee, f_stair2price f_stair2price, f_stair2amount f_stair2amount,"
				...
				+ " min(u.f_stair3price) f_stair3price,"
				+ " Round(SUM(isnull(h.f_stair3amount,0)),2) f_stair3amount, Round(SUM(isnull(h.f_stair3fee,0)),2) f_stair3fee,"
		...
		/*
		 * for (Object obj : list) { Object[] data = (Object[])obj;
		 * map.put("f_stair1price", data[0]); map.put("f_stair1amount",
		 * data[1]); map.put("f_stair1fee", data[2]); map.put("f_stair2price",
		 * data[3]); map.put("f_stair2amount", data[4]); map.put("f_stair2fee",
		 * data[5]); map.put("f_stair3price", data[6]);
		 * map.put("f_stair3amount", data[7]); map.put("f_stair3fee", data[8]);
		 * }
		 */
		return map;
	}
